...
write_run_navigation <- function(state, report) {
  ...
  for (id in ids) {
    ...
    components[[id]] <- list(component_id = id, source = source,
      ...
      revision_id = rev$revision_id %||% NULL, dependencies = I(deps),
  ...
  manifest <- list(schema_version = 1L, run_id = report$run_id, status = report$status,
    status_reason = report$status_reason, selected_attempt_id = report$selected_attempt_id,
  ...
  writeLines(c('<!doctype html><html lang="en"><meta charset="utf-8">',
    ...
    paste0('<p class="status">', run_html_escape(report$status), '</p>'),
    paste0('<p>', run_html_escape(report$status_reason), '</p>'),
