...
#' One guarded line at the top of each generated program sources `autoexec.R`
#' into the program's own environment, the way a SAS session runs its autoexec
#' first: a program runs from its folder -- `Rscript adsl.R` there, or
#' `source("adsl.R")` with the run folder as the working directory -- or after
#' `source("<run folder>/autoexec.R", chdir = TRUE)` was run once in the
#' session, after which every program finds the runtime loaded and pasted code
#' works too. `autoexec.R` is the bundle's
