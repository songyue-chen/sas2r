...
#' @param output_root Root for newly assigned libraries' generated outputs.
#' @return Invisible destination path.
#' @noRd
write_autoexec <- function(project, out_dir, effective = effective_librefs(project),
                           library_map = NULL, output_root = out_dir, organized = FALSE) {
  ...
  if (isTRUE(organized)) {
    entries <- vapply(names(configured), function(libref) {
      entry <- configured[[libref]]
      sprintf("  %s = list(read_path = %s, write_path = file.path(.sas2r_output_root, \"datasets\", %s), engine = %s, write = %s),",
        libref_list_key(libref), registry_path_literal(entry$read_path %||% entry$path, root),
        deparse(libref), deparse(entry$engine %||% "sas7bdat"), deparse(entry$write %||% "rds"))
    }, character(1), USE.NAMES = FALSE)
    work_entry <- '  work = list(read_path = file.path(.sas2r_output_root, "work"), write_path = file.path(.sas2r_output_root, "work"), engine = "rds", write = "rds")'
  }
  output_root <- normalizePath(output_root, winslash = "/", mustWork = FALSE)
  output_root <- if (identical(output_root, root)) "." else
    if (identical(output_root, dirname(root))) ".." else output_root
  writeLines(autoexec_lines(entries, undec_entries, work_entry,
                           execution_root = project$project_dir,
                           output_root = output_root, organized = organized),
...
#' @noRd
autoexec_lines <- function(entries, undeclared, work_entry, execution_root = NULL,
                           output_root = NULL, organized = FALSE) {
  version <- tryCatch(as.character(getNamespaceVersion("sas2r")),
                      error = function(e) "unknown")
  runtime_dir <- if (organized) "runtime/" else ""
  helpers <- paste0(runtime_dir, "sas2r-helpers.R")
  formats <- paste0(runtime_dir, "_sas2r_formats.R")
  output_lines <- c(
    '.sas2r_bundle_root <- normalizePath(getwd(), winslash = "/", mustWork = FALSE)',
    if (organized) c(
      '# Change this write root to keep separate manual runs.',
      '.sas2r_output_root <- file.path(.sas2r_bundle_root, "output")',
      '.sas2r_program_root <- file.path(.sas2r_bundle_root, "programs")'
    ) else paste0('.sas2r_output_root <- ', if (is.null(output_root)) '.sas2r_bundle_root' else
      paste0('normalizePath(', if (output_root %in% c(".", ".."))
        paste0('file.path(.sas2r_bundle_root, ', deparse(output_root), ')') else deparse(output_root),
  ...
  c(
    ...
    "# and uncomment it.",
    "",
    "# Relative paths in translated LIBNAME calls use this project execution root.",
    "# Update it when moving the source project; NULL uses this bundle's folder.",
    paste0(".sas2r_execution_root <- ",
           if (is.null(execution_root)) "NULL" else deparse(normalizePath(execution_root, winslash = "/", mustWork = FALSE))),
    "",
    output_lines,
    "",
    "# ---- LIBRARIES --------------------------------------------------------------",
    ...
    sprintf('if (!file.exists(%s))', deparse(helpers)),
    '  stop("autoexec.R: source() it with chdir = TRUE, or setwd() into its folder first",',
    "       call. = FALSE)",
    'if (is.null(.sas2r_execution_root)) .sas2r_execution_root <- .sas2r_bundle_root',
    ...
    'local({',
    sprintf('  for (file in list.files(%s, pattern = "\\\\.R$", full.names = TRUE))',
      deparse(if (organized) "macros" else "R/macros")),
