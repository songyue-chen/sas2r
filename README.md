...
- **Standalone R programs you can take anywhere.** The result is a directory of plain R scripts plus a few small runtime files, and the folder can be moved as a whole. They run on Posit Workbench, a laptop, a server, or a batch system — without `sas2r` installed. The runtime is documented and versioned: see `?sas2r_runtime` and `vignette("runtime-helpers")`.
