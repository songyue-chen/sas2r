...
- **Standalone R programs you can take anywhere.** The result is a directory of plain R scripts plus one small helper file. They run on Posit Workbench, a laptop, a server, or a batch system — without `sas2r` installed. The helper file is a documented, versioned runtime: see `?sas2r_runtime` and `vignette("runtime-helpers")`.
