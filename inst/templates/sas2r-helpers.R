...
# trailing blanks, and a missing character value ("" or NA) sorts below every
# non-missing one; numeric missing sorts below every number. With `op` NULL the
# result is -1/0/1 (missing equals missing); with an operator ("==", "<", ...)
# it is the logical SAS would produce, never NA.
...
# Row-wise mean of the non-missing arguments; see sas_sum().
sas_mean <- function(...) {
  args <- list(...)
  if (length(args) == 0L) return(numeric(0))
  n <- max(vapply(args, length, integer(1)))
...
# SAS COMPRESS: remove every character listed in `chars` (a regex character
# class, so a "]" or "-" in the list needs escaping).
sas_compress <- function(x, chars = " ") gsub(paste0("[", chars, "]"), "", x)

# SAS SUBSTR(x, pos, len): `len` NULL reads to the end of the string.
...
# Row-wise maximum across the arguments, ignoring missing; all-missing yields
# missing, and a single argument passes through elementwise. See sas_min().
sas_max <- function(...) {
  args <- list(...)
  if (length(args) == 0L) return(numeric(0))
  if (length(args) == 1L) return(args[[1]])
...
# SAS LENGTH: trailing blanks do not count, and a missing or blank value has
# length 1, as in SAS.
...
# SAS PUT(x, fmt.) with a format from the bundle's _sas2r_formats.R catalog.
...
# MERGE a b; BY by; with IN= semantics selected by `keep`: "both" (in_a and
# in_b), "left" (in_a), "right" (in_b), "left_only", "right_only", "full".
# Where both datasets carry a non-key column the later dataset's value wins,
# columns follow statement order, and rows follow SAS BY ordering. A
# many-to-many key is refused: no join reproduces SAS's row walking there.
...
# Apply a compiled format catalog entry: exact `values`, then `ranges`
# (lo..hi -> label), then `other` for anything unmatched. Missing input stays
# missing unless the format defines `other`.
...
# LIBNAME libref CLEAR: the libref stops resolving from this point on.
...
# Compatibility alias kept for bundles generated by earlier versions; new
# code calls sas_sort() directly. Scheduled for retirement (ADR 0003).
...
# Display text the way SAS prints values: numeric missing is ".", character
# missing is blank, numbers are never in scientific notation.
