test_that("a project configuration update inherits omitted fields and replaces whole fields", {
  ...
  first <- sas_preflight(file, diagnose = "off", config = cfg,
  ...
  updated <- sas_preflight(first$project, diagnose = "off", config = list(comparison_rules = list(min_rows = 10)))
  ...
  model_update <- sas_preflight(updated$project, diagnose = "off", config = list(llm = list(provider = "openai", model = "changed")))
  ...
  expect_identical(sas_preflight(model_update$project, diagnose = "off", config = list())$project$config, model_update$project$config)
})
...
test_that("translation accepts project updates and explicit NULL resets a field", {
  ...
  first <- sas_preflight(file, diagnose = "off", config = list(libraries = list(adam = "."),
    llm = list(provider = "openai", model = "unused")),
  ...
  cleared <- sas_preflight(result$project, diagnose = "off", config = list(comparison_rules = NULL))
...
test_that("explicit scan changes still require a rescan and complete configs replace", {
  ...
  for (field in names(scan_config_fields(first$project$config))) {
    for (value in list(NULL, list(), "changed")) {
      ...
      expect_error(sas_preflight(first$project, diagnose = "off", config = patch), class = "sas2r_config_error")
  ...
  expect_identical(sas_preflight(first$project, diagnose = "off", config = replacement)$project$config$comparison_rules,
    list(min_rows = 3))
  replacement$libraries <- NULL
  expect_error(sas_preflight(first$project, diagnose = "off", config = replacement), class = "sas2r_config_error")
  yaml_path <- file.path(root, "replacement.yml")
  writeLines("comparison_rules: {min_rows: 3}", yaml_path)
  expect_error(sas_preflight(first$project, diagnose = "off", config = yaml_path), class = "sas2r_config_error")
  expect_no_error(sas_preflight(file, diagnose = "off", config = yaml_path))
})
