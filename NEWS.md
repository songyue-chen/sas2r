...
### Runtime
* **One boot file per bundle, and a short program header**: every bundle now carries `_sas2r_boot.R`, which loads the registry, helpers, and formats in order into the environment it is sourced into. A translated program opens with a 24-line block that locates the boot file from the program's own path and sources it, instead of a 55-line block that did all of that inline; the launch modes are unchanged (`source()`, `sys.source()`, `Rscript`, `R CMD BATCH` from any working directory, and `setwd()` for pasted code), and so is the environment model: a program sourced into a sandbox keeps its runtime there. Sourcing the boot file by hand once per session is the R counterpart of a SAS autoexec -- programs then find the runtime loaded and skip their own bootstrap. The registry records paths inside the bundle (the session `work` library and every library the programs write) relative to `.sas2r_bundle_root`, which the boot file binds to its own folder, so a run folder moved or renamed as a whole keeps working; paths outside the bundle -- the source data programs read -- stay absolute. The bundle executor loads a bundle through its boot file too.
