...
Each attempt runs in an isolated directory, grouped per run (`attempts/<run_id>/bundle_attempt_NNN/`, where run ids are timestamp-first and sort chronologically) so repeated runs into the same output directory never collide. Every run folder also carries its own `report.md` and `report.json`; the `report.md` at the output root is always the latest run's copy:
