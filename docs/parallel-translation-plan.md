...
Progress: configuration, process workers, parallel final reviews, and dependency-ready component generation/review are implemented. The async probe exposed a request-settings blocker: a scoped 20 ms timeout rejected a synchronous 150 ms replay request, but the equivalent asynchronous request completed after the options scope had ended. Fresh `callr` workers preserve the existing synchronous request settings and conversation lifecycle. This is evidence against the current scoped-options integration, not a claim that ellmer cannot support an alternative async implementation. No second production transport was added.
...
- The coordinator owns budget admission, revision selection and checkpoints. Each native-tool HTTP continuation now returns through the same admission path in both modes. Provider-neutral tool requests/results are retained, but provider-native reasoning content and thought signatures still need preservation and explicit Gemini/DeepSeek continuation replay coverage. This is a remaining INV-001 transport gap even with one workflow; the existing OpenAI-style replay does not establish the other providers' continuation contract. Custom adapters without a reconstruction factory, and shipped adapters configured with internal transport retries (`max_tries > 1`), visibly fall back to one worker. They are not silently reconfigured.
...
- Version-9 checkpoints save component stages and revisit counts and import compatible version-8 work. Legacy revisit counts remain unknown. Interrupted requests retain unknown costs and strict reservations rather than receiving a speculative refund.
- Uncertain dependency findings defer the affected branch; independent components continue. Automatic graph reconciliation and the new reporting tool remain step 5 in a separate follow-up. Until then, the full dynamic-recovery feature is **not complete**.
